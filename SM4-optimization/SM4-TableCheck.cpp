...
//左移
u32 ShiftLeft(u32 m, int len) {
    return ((m << len) & 0xFFFFFFFF) | ((m & 0xFFFFFFFF) >> (32 - len));
}

//查表8bit到32bit的T函数优化
u32 functionT1(u32 b) {
    u8 a[4];
    short i;
    a[0] = b;
    a[1] = b >> 8;
...
u32 functionT2(u32 b) {
    ...
    b = b ^ ShiftLeft(b, 13) ^ ShiftLeft(b, 23);
    return b;
}

//密钥扩展算法
void RoundKey(u32 MK[], u32 K[], u32 RK[]) {
    for (int i = 0; i < 4; i++) {
        K[i] = MK[i] ^ FK[i];
    }
    for (int i = 0; i < 32; i++) {
        K[(i + 4) % 4] = K[i % 4] ^ functionT2(K[(i + 1) % 4] ^ K[(i + 2) % 4] ^ K[(i + 3) % 4] ^ CK[i]);
        RK[i] = K[(i + 4) % 4];
    }
}
...
//加密算法
void en_SM4(u32 X[], u32 RK[], u32 Y[]) {
    for (int i = 0; i < 32; i++) {
        X[(i + 4) % 4] = X[i % 4] ^ functionT1(X[(i + 1) % 4] ^ X[(i + 2) % 4] ^ X[(i + 3) % 4] ^ RK[i]);
    }
    for (int i = 0; i < 4; i++) {
        Y[i] = X[4 - 1 - i];
    }
...
//解密算法
void de_SM4(u32 X[], u32 RK[], u32 Y[]) {
    short i;
    u32 TEMP[32];
    for (int i = 0; i < 32; i++) {
        TEMP[i] = RK[32 - 1 - i];
    ...
    for (int i = 0; i < 32; i++) {
        X[(i + 4) % 4] = X[i % 4] ^ functionT1(X[(i + 1) % 4] ^ X[(i + 2) % 4] ^ X[(i + 3) % 4] ^ TEMP[i]);
    }
    for (int j = 0; j < 4; j++) {
        Y[j] = X[4 - 1 - j];
    }
...
int main(void) {
    ...
    u32 K[4];
    printf("明文：");
    scanf_s("%8x%8x%8x%8x", &M[0], &M[1], &M[2], &M[3]);
    printf("密钥：");
